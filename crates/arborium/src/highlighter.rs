...
"#;
    ...
    #[cfg(feature = "lang-rust")]
    fn test_showcase() {
        let rust_code = r#"/// A recursive fibonacci implementation
...
fn main() {
    ...
    println!("fib(10) = {}", result);
        ...
        let themes: &[(&str, fn() -> &'static arborium_theme::Theme)] = &[
        ...
        println!("\n{}", "═".repeat(70));
        println!("  ARBORIUM SHOWCASE - Multiple Themes");
        println!("{}\n", "═".repeat(70));

        // Plain highlighting (no box)
        println!("── Plain Output ──\n");
        let theme = builtin::catppuccin_mocha().clone();
        let mut hl = AnsiHighlighter::new(theme);
        let output = hl.highlight("rust", rust_code).unwrap();
        println!("{output}\n");

        // With box and padding
        println!("── With Border & Padding ──\n");
        for (name, theme_fn) in themes.iter().take(4) {
            println!("  Theme: {name}\n");
