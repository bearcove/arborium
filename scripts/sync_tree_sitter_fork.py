...
def patch_binding_rust_build_rs(target: Path) -> None:
    ...
    Strategy:
    - Start from whatever upstream build.rs currently is.
    - Insert target-family wasm include path hook.
    - Keep existing wasm32 unknown handling if present.
    """
    ...
    marker = "DEP_ARBORIUM_SYSROOT_PATH"
    if marker in src:
        info("build.rs already has arborium sysroot patch.")
        return

    insert_block = """
    // Arborium patch: for wasm targets, add wasm-sysroot stub headers.
    // Implementations are provided by the `arborium-sysroot` crate (links = "arborium_sysroot").
...
"""
    ...
    anchor = "for entry in fs::read_dir(&src_path).unwrap() {"
    idx = src.find(anchor)
    if idx == -1:
        warn("Could not find insertion anchor in build.rs; writing unchanged.")
        return
